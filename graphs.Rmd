...
```{r target vs sib gender}

genderdf_long <- sibship_factors %>%
  select(Target_Gender, Sib_Gender) %>%
  pivot_longer(
    cols = c(Target_Gender, Sib_Gender),
    names_to = "Subject",
    values_to = "Gender"
  )

genderdf_long <- genderdf_long %>%
  filter(Gender != "n/a")

genderplot <- ggplot(genderdf_long, aes(x = Gender, fill = Subject)) +
  ...
  geom_text(stat = "count", aes(label = after_stat(count),
                                vjust = -0.3,
                                family = "Times")) +
  scale_fill_manual(values = c("Sib_Gender" = "plum", "Target_Gender" = "skyblue")) +
