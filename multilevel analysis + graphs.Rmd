...
```{r combining comprehended + produced WG new column, echo=FALSE, message=FALSE,warning=FALSE, fig.align='center', fig.width=6, fig.height=2.5}
combined$`Words Understood` <- as.numeric(combined$`Words Understood`)
combined$`Words Produced` <- as.numeric(combined$`Words Produced`)
combined$WG_standardized<- rowSums(combined[, c("Words Understood", "Words Produced")], na.rm = TRUE)

```

```{r words-produced graph, echo=FALSE, message=FALSE,warning=FALSE, fig.align='center', fig.width=6, fig.height=2.5}

ggplot(combined, aes(x = WG_standardized)) + 
  geom_histogram(binwidth = 50, fill = "lightpink") +
  scale_y_continuous(limits = c(0, 10)) +
  labs(
    title = "Distribution of 'Words Produced & Comprehended in 11 to 19m Children'",
    x = "Production & Comprehension",
    y = "Frequency"
...
combined$WG_standardized <- as.numeric(as.character(combined$WG_standardized))

ggplot(
  data = combined %>%
    filter(!is.na(age), !is.na(WG_standardized), WG_standardized != 0, age <= 19),
  aes(x = age, y = WG_standardized)
) +
  geom_jitter(width = 0.2, height = 10, alpha = 0.6, size = 2) +
  geom_smooth(method = "loess", se = TRUE, color = "forestgreen", size = 1.2) +
  labs(
    title = "MBCDI Words Produced/Comprehended (WG) by Child Age",
    x = "Child Age (months)",
    y = "Words Produced"
  ) +
  theme_minimal(base_size = 10, base_family = "Times")
...
Due to the low number of observations in the individual younger and older sibling categories, the identities were combined into broader groups (“Older Siblings” and “Younger Siblings”) to improve interpret ability and ensure sufficient sample size for analysis.
...
#### Production, Correlation, and Parent/Child Score by Sibling Identity
...
sibship_factors <- sibship_factors %>% 
  left_join(
    combined
    %>%
      select(subject_id, child_score, parent_score, `Words Understood`, `Words Produced`, `Total Produced`),
    by = c("Subject_ID" = "subject_id")
  ) 

sibship_factors$WG_standardized <- rowSums(sibship_factors[, c("Words Understood.x", "Words Produced")], na.rm = TRUE)

sibship_factors <- sibship_factors %>%
  mutate(Produced = coalesce(`WG_standardized`, `Total Produced`)) %>% filter(!is.na(Produced), !is.na(Sib_Identity))

sibship_factors <- sibship_factors %>%
  left_join(
    combined %>% select(subject_id, child_score, parent_score, `WG_standardized`, `Total Produced`),
    by = c("Subject_ID" = "subject_id")
  ) 

ggplot(sibship_factors, aes(x = Sib_Identity, y = `WG_standardized`)) +
...
sib_clean <- sib_clean %>%
  mutate(Produced = coalesce(`WG_standardized`, `Total Produced`)) %>% filter(!is.na(Produced), !is.na(Sib_Group))

ggplot(sib_clean, aes(x = Sib_Group, y = `Produced`)) +
  geom_boxplot(outlier.shape = NA, fill = "lightpink", alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.5, color = "grey40") + 
  labs(
    title = "Words/Total Produced by Sibling Identity",
...
Multilingual households were also looked at. Out of 65 families, only 19 have another language, that is not English, spoken at home. The responses were also categorized by language group. `Multilingual` refers to households that speak more than one language group. 
...
ggplot(lang_yesno, aes(x = lang_binary, y = Count, fill = lang_binary)) +
  ...
  labs(
    title = "Another Language at Home?",
    x = "Response",
    y = "Count"
  ) +
...
Post-hoc comparisons using Tukey’s HSD test indicated that children with a younger sister (M = …) produced significantly more words than those with an older brother, p = .040, and those with an older sister, p = .028. No other pairwise comparisons were significant. These results should be interpreted with caution, as the younger sibling groups currently have small sample sizes, and data collection is ongoing.
...
```{r wilcox test sib child/parent score, echo=FALSE, message=FALSE,warning=FALSE, fig.align='center', fig.width=5, fig.height=3}

sib_parentchild_WT <- wilcox.test(
  Produced ~ `Have Sib?`,
  data = sib_df,
  exact = FALSE
)
paste0("W = ", round(sib_parentchild_WT$statistic, 2), 
       ", p = ", signif(sib_parentchild_WT$p.value, 3))
```
