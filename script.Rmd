...
Below data mining additions as of 7/7/25 (not done yet)
```{r}
# Setting up new data

setwd("/Volumes/Fas-Phyc-PEB-Lab/experimental_projects/PBS/PBS_Full_Study/CN_Working_Folder/R/Survey/7:7:25")

Qualtrics_7.7 <- read_excel("PBS CHS - Qualtrics_7.7.xlsx")
WS_7.7 <- read_excel("PBS CHS - WS_7.7.xlsx")
WG_7.7 <- read_excel("PBS CHS - WG_7.7.xlsx")

colnames(Qualtrics_7.7) <- as.character(Qualtrics_7.7[1, ])
Qualtrics_7.7 <- Qualtrics_7.7[-1, ]

# Cleaning
WS_7.7 <- WS_7.7[, -c(5:10)]
WG_7.7 <- WG_7.7[, -c(5:10)]
Qualtrics_7.7 <- Qualtrics_7.7[, -c(1:16)]
Qualtrics_7.7 <- Qualtrics_7.7[-c(1:12),]
WG_7.7 <- WG_7.7[, -18]


Qualtrics_7.7 <- Qualtrics_7.7 %>% select(where(~!all(is.na(.x))))
WS_7.7 <- WS_7.7 %>% select(where(~!all(is.na(.x))))
WG_7.7 <- WG_7.7 %>% select(where(~!all(is.na(.x))))


Qualtrics_7.7[] <- lapply(Qualtrics_7.7, as.character)
combined[] <- lapply(combined, as.character)
WG_7.7[] <- lapply(WG_7.7, as.character)
WS_7.7[] <- lapply(WS_7.7, as.character)
```
Combining new entries into existing combined dataset (work in progress)
```{r}
